...
checkpoint_path = os.path.join("checkpoint", "RNN_10F.h5")

# Load and prepare data
df = pd.read_csv(CSV_PATH, parse_dates=["datetime"], index_col="datetime").sort_index()

# Keep required raw columns and add engineered features
...
print("RNN shapes:", X_train_np.shape, y_train_np.shape, X_test_np.shape, y_test_np.shape)

# Build model
model = Sequential([
    LSTM(128, input_shape=(LOOK_BACK, N_FEATURES), return_sequences=False),
    Dropout(0.2),
...
print("Saved RNN to", checkpoint_path)
